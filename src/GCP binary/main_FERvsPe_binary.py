...
from GCP_Encode_binary import GCP_Encode_binary_brute, GCP_Encode_binary_rep
from GCP_Decode_binary import GCP_Decode_binary_brute, GCP_Decode_binary_rep
from preCompute_Patterns import preCompute_Patterns
from GCP_theoretical_binary import theoretical_error_brute, theoretical_error_rep
from binary_channel import binary_channel

# ---------------- Code configuration (fixed across scenarios) ----------------
k = 140
c1 = 8
c2 = 1
l  = 7
len_last = (k - 1) % l + 1
K = int(math.ceil(k / l))
opt = True  # optimizes but can slow decoding when True
...
total_iter = 5000                          # total trials per Pe (per scenario)
Pe_values = np.arange(0.001, 0.010, 0.001)  # grid of Pe
...
def init_worker(patterns, codebook, mode, split_tuple, t_rep_val):
    ...
    global Patterns, codebook_binary, MODE, split, t_rep_global
    Patterns = patterns
    codebook_binary = codebook
    MODE = mode
    split = split_tuple
...
def process_chunk(args):
    ...
    for _ in range(chunk_size):
        ...
        if MODE.lower() == "brute":
            x, n, N, K_enc, q, U, X, check_par = GCP_Encode_binary_brute(u, l, c1, codebook_binary)
        else:
            x, n, N, K_enc, q, U, X, check_par = GCP_Encode_binary_rep(u, l, c1, c2, t_rep_global)

        # channel 
        y = binary_channel(n, x, Pd, Pi, Ps)

        # decode
        t0 = time.perf_counter()
        if MODE.lower() == "brute":
            uhat, check_par_hat = GCP_Decode_binary_brute(
                y, n, k, l, N, K_enc, c1, q, len_last, lim, Patterns, codebook_binary, d_min, opt
            )
        else:
            uhat, check_par_hat = GCP_Decode_binary_rep(
...
# -------------- Per-scenario runner --------------
def run_case(MODE, param2, split, Patterns, codebook_binary):
    """
    param2:
    ...
    n_check = len(codebook_binary[0]) if (MODE.lower() == "brute" and codebook_binary is not None) else 0

    # Code rate / case tag
    if MODE.lower() == "brute":
        code_rate = k / (k + c1*l + n_check)
    ...
    tqdm.write(f"Coderate: {code_rate:.3f}")
    ...
    tqdm.set_lock(RLock())
    desc = (
        f"Simulating [{CASE_TAG}, "
        f"split=({split[0]:.2f},{split[1]:.2f},{split[2]:.2f}), "
        f"opt={OPT_TAG}]"
    )
    with Pool(
        processes=NPROCS,
        initializer=init_worker,
        initargs=(Patterns, codebook_binary if MODE.lower()=="brute" else None, MODE, split, t_rep_case if t_rep_case is not None else 0)
    ) as pool:
        for pe, S, F, E, P, t_ in tqdm(
            pool.imap_unordered(process_chunk, tasks, chunksize=1),
            total=len(tasks), desc=desc, unit="task", dynamic_ncols=True
        ):
    ...
    out_path = os.path.join(CURRENT_DIR, "Results", "results_binary.txt")
    with open(out_path, "a", encoding="utf-8") as f:
        ...
        f.write(f"Coderate: {code_rate:.3f}\n")
    ...
    pdf_name = os.path.join(
        "Results",
        f"FER_vs_Pe_binary_{MODE}_t{t_rep_case if MODE=='rep' else 'NA'}_"
        f"split{split_tag(split)}_opt{OPT_TAG}_{ts}.pdf"
    )
    os.makedirs(os.path.join(CURRENT_DIR, "Results"), exist_ok=True)
...
if __name__ == '__main__':
    ...
    codebook_path = os.path.join(CURRENT_DIR, "codebook_binary.txt")
    if os.path.exists(codebook_path):
        with open(codebook_path, "r", encoding="utf-8") as f:
            codebook_global = [[int(c) for c in line.strip()] for line in f if line.strip()]
    else:
